 ...
 * command. A session app:up started cannot end there — the boot behind it
 * owns a server that has to be stopped again when the terminal quits, and an
 * exec'd process leaves no PHP behind to stop it. So app:up claims the
 * session first, and `dev` hands the terminal to a child process instead,
 * returning control when it exits.
 *
 * Shared state because the claim crosses a command boundary: app:up sets
